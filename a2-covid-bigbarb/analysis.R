...
# https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-covid.csv
national <- read.csv("https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-covid.csv",stringsAsFactors = FALSE)

# 1.c Load the *state level* data from the following URL into a variable called `states`
# https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-states-covid.csv
states <- read.csv("https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-states-covid.csv", stringsAsFactors = FALSE)
...
# https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-counties-covid.csv
counties <- read.csv("https://github.com/melaniewalsh/Neat-Datasets/raw/main/us-counties-covid.csv", stringsAsFactors = FALSE)
...
national <- national %>%
  mutate(new_cases = cases - lag(deaths, default = 0))
...
# HINT: The dyplr lag() function will be very helpful here.
national <- national %>%
  mutate(new_deaths = cases - lag(deaths, default = 0))
...
all_totals <- left_join(total_cases_counties, national, na.rm = TRUE)
  view(all_totals)
...
national_county_diff <- subset(all_totals, county_total_cases != cases)
...
# How many covid cases are in Maricopa County, Arizona?
#

#  Write code (at least 2-3 lines) that will answer your question
my_answer <- counties %>% 
  filter(date == max(date), na.rm =TRUE) %>% 
  filter(location == ("Maricopa, Arizona")) %>% 
  pull(cases)
